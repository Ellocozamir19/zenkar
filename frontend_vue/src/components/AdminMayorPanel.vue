<template>
  <div>
    <div style="background:#fffbe6;color:#b91c1c;padding:8px 16px;font-size:13px;">
      <!-- DEBUG: Mostrar usuario y permiso -->
      <b>DEBUG:</b> currentUser: {{ currentUser }}, hasPermission: {{ hasPermission }}
    </div>
    <div v-if="hasPermission" class="admin-mayor-panel-full">
      <div class="admin-mayor-panel-container">
        <!-- Header del panel admin (NO el global de la app) -->
        <header class="admin-mayor-header" style="margin-top: 12px;">
          <h1>Panel de Administración - Admin Mayor</h1>
        </header>
        <!-- Tabs y Add Button en la misma fila -->
        <div class="admin-mayor-tabs-row">
            ...
            <table>
              ...
              <tbody>
                <tr v-for="vehiculo in filteredVehiculos" :key="vehiculo.id">
                  <td>
                    <div class="user-cell">
                      <div class="admin-mayor-user-avatar-sm">{{ vehiculo.marca?.charAt(0) }}</div>
        ...
        <div v-if="showModal" class="admin-modal-overlay">
          <div class="admin-modal" style="max-width:680px; min-width:500px; width:96vw; max-height:90vh; overflow-y:auto; display:flex; flex-direction:column; justify-content:flex-start; align-items:center; margin-top:70px;">
              ...
              <div class="admin-modal-body" style="overflow-y:auto;max-height:60vh;width:100%;padding-bottom:1rem;">
                <div v-if="modalType==='usuario'">
                  ...
                  <select v-model="formData.tipo" required>
                    ...
                    <option value="Admin Mayor">Admin Mayor</option>
                  </select>
        ...
        <div v-if="showVehiculoInfo" class="admin-modal-overlay">
          <div class="admin-modal admin-mayor-vehiculo-info-modal">
            <div class="admin-mayor-vehiculo-info-header">
              <div style="display:flex;align-items:center;gap:0.7rem;">
                <img :src="infoIcon" alt="Detalles" />
                <h3>Detalles del Vehículo</h3>
              </div>
            </div>
            <div class="admin-mayor-vehiculo-info-body">
              <template v-if="vehiculoInfo">
                <div v-for="key in [
                  ...
                  <template v-if="key === 'imagen' && typeof vehiculoInfo[key] === 'string' && vehiculoInfo[key]">
                    <div class="admin-mayor-vehiculo-img">
                      <img :src="vehiculoInfo[key]" alt="Imagen del vehículo" />
                    </div>
                  </template>
                  <template v-else>
                    <div class="vehiculo-details-row">
                      <span class="admin-mayor-vehiculo-label">{{ key.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()) }}:</span>
                      <span class="admin-mayor-vehiculo-value">{{ vehiculoInfo[key] === true ? 'Sí' : vehiculoInfo[key] === false ? 'No' : (vehiculoInfo[key] ?? '-') }}</span>
                    </div>
                  </template>
                </div>
              </template>
            ...
            <div class="admin-mayor-vehiculo-info-actions">
        ...
        <div v-if="showUserDetails" class="admin-modal-overlay">
          <div class="admin-modal user-details-modal" style="max-width:440px;min-width:300px;background:#fff;border-radius:16px;box-shadow:0 4px 32px rgba(0,0,0,0.09);padding:0;overflow:hidden;">
            ...
            <div class="admin-modal-actions user-details-actions" style="padding:1rem 0 0.5rem 0;display:flex;justify-content:center;align-items:center;background:#f3f4f6;border-top:1px solid #e5e7eb;">
...
auth.loadUser()
const currentUser = computed(() => auth.user || JSON.parse(localStorage.getItem('user') || '{}'))
const hasPermission = computed(() => (currentUser.value?.tipo || currentUser.value?.tipo_usuario) === 'admin_mayor')
...
function openModal(type: 'usuario'|'vehiculo', item: any = null) {
  ...
  // Limpiar y preparar formData según tipo
  if (item) {
    formData.value = { ...item }
    // Si es edición de vehículo, setear imagenTipo según qué campo tiene valor
    if (type === 'vehiculo') {
      if (item.imagen_archivo) {
        formData.value.imagenTipo = 'archivo'
        formData.value.imagen = ''
        formData.value.imagenPreview = item.imagen_archivo
      } else if (item.imagen) {
        formData.value.imagenTipo = 'url'
        formData.value.imagenPreview = ''
      } else {
        formData.value.imagenTipo = ''
        formData.value.imagenPreview = ''
      }
    }
  } else {
...
async function handleSubmit() {
  if (modalType.value === 'usuario') {
    ...
    try {
      ...
      // Si es archivo, subir primero la imagen y poner la URL devuelta en 'imagen'
      ...
      // Si es URL, ya está en payload.imagen
      // En cualquier caso, nunca enviar imagen_archivo
...
async function toggleAdmin(id: number) {
  ...
  if (usuario.tipo === 'Admin') {
    ...
    nuevoTipo = 'Admin'
    tipo_usuario = 'admin'
  } else if (usuario.tipo === 'Admin Mayor') {
    nuevoTipo = 'Usuario'
    tipo_usuario = 'usuario'
  }
  try {
...
// Métodos de permisos: SIEMPRE true para admin mayor
const canEditUser = () => true
const canDeleteUser = () => true
const canChangeRole = () => true

const expandedVehiculo = ref<number|null>(null)
function toggleExpandVehiculo(id: number) {
  expandedVehiculo.value = expandedVehiculo.value === id ? null : id
}
...
// Lógica para abrir/cerrar detalles
